...
def extract_category_ids(fsq_category_ids):
    """Extract category IDs from fsq_category_ids column, handling space-separated values."""
    ec = -1
    fsq_len = len(str(fsq_category_ids))
    try:
        if pd.isna(fsq_category_ids):
            ec = 1
            return []
        elif isinstance(fsq_category_ids, list):
            ec = 2
            return fsq_category_ids  # Already a list, return as is
        elif isinstance(fsq_category_ids, str):
            ec = 3
            # Handling space-separated string format
            return fsq_category_ids.split()  # Split on space to get individual IDs
        elif len(fsq_category_ids) == 0:
            ec = 4
            return fsq_category_ids
        elif len(fsq_category_ids) > 0:
            ec = 5
            print('xxx')
            return str(fsq_category_ids[1:-1]).split(" ")
        else:
            ec = 99
            return []  # Default case
    except Exception as e:
        print(f"Error processing value: {fsq_category_ids}, error code {ec}, fsq length {fsq_len}")
        raise e

# Apply the extraction function with a try-except block for debugging
places['fsq_category_ids'] = places['fsq_category_ids'].apply(lambda x: extract_category_ids(x))
