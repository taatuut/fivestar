...
places = pd.read_parquet(places_file, engine="pyarrow", columns=["fsq_category_ids", "latitude", "longitude", "country"], nrows=10000)
...
def extract_category_ids(fsq_category_ids):
    """Extract category IDs from fsq_category_ids column."""
    if pd.isna(fsq_category_ids) or not fsq_category_ids:
        return []
    try:
        # Parse the array of single-quoted, comma-separated values
